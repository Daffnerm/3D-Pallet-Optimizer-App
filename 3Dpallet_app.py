...
# User inputs (all integer-only, with new defaults)
...
def plot_cube(ax, x=0, y=0, z=0, dx=1, dy=1, dz=1, color='red'):
    ...
    faces = [
        [vertices[0], vertices[1], vertices[2], vertices[3]],  # bottom face (z=0)
        [vertices[4], vertices[5], vertices[6], vertices[7]],  # top face (z=dz)
        [vertices[0], vertices[1], vertices[5], vertices[4]],  # front face (y=0)
        [vertices[1], vertices[2], vertices[6], vertices[5]],  # right face (x=dx)
        [vertices[2], vertices[3], vertices[7], vertices[6]],  # back face (y=dy)
        [vertices[3], vertices[0], vertices[4], vertices[7]]   # left face (x=0)
    ]
    
    # Create the 3D polygons
    collection = Poly3DCollection(faces, linewidths=1, edgecolors='k')
...
if carton_height > max_height:
    ...
    with vis_col1:
        ...
        # Generate positions for the cartons
        positions = []
    ...
    with vis_col2:
        # 3D Visualization - NEW IMPLEMENTATION
        if cartons_per_layer > 0:
            fig_3d = plt.figure(figsize=(6, 6))
            ax3d = fig_3d.add_subplot(111, projection='3d')
            
            # First set correct limits
            stack_height = PALLET_HEIGHT + (max_layers * carton_height)
            ax3d.set_xlim(0, PALLET_LENGTH)
            ax3d.set_ylim(0, PALLET_WIDTH)
            ax3d.set_zlim(0, stack_height)
            
            # Draw wooden pallet at the bottom (z=0)
            plot_cube(
                ax3d, 
                x=0, y=0, z=0, 
                dx=PALLET_LENGTH, dy=PALLET_WIDTH, dz=PALLET_HEIGHT, 
                color='#C0A080'  # Wooden color
            )
            
            # Draw cartons layer by layer
            for layer in range(max_layers):
                z_pos = PALLET_HEIGHT + (layer * carton_height)  # Start above the pallet
                
                # Alternate layer colors
                color = '#2E86C1' if layer % 2 == 0 else '#3498DB'
                
                # Draw each carton in this layer
                for pos in positions:
                    ...
                    plot_cube(
                        ...
                        dx=box_w, dy=box_h, dz=carton_height,
                        color=color
                    )
            ...
            ax3d.set_box_aspect((PALLET_LENGTH, PALLET_WIDTH, stack_height))
    ...
    stack_volume = pallet_area * (max_layers * carton_height)  # Volume without pallet base
