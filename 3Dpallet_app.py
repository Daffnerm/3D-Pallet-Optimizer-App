...
def calculate_capacity(l, w, h, max_h):
    ...
    # Choose best result
    cartons_per_layer = max(theoretical_max, len(rects))
    use_original = orient1 >= orient2 if cartons_per_layer == theoretical_max else False
    
    return cartons_per_layer, use_original, rects
...
if carton_height > max_height:
            ...
            for layer in range(max_layers):
                ...
                if (carton_length == 17 and carton_width == 13) or (carton_length == 13 and carton_width == 17):
                    for pos in positions:
                        dx = pos["w"]
                        dy = pos["h"]
                        draw_box(ax3d, pos["x"], pos["y"], z_offset, dx, dy, carton_height, color)
                else:
                    for rect in rects:
                        draw_box(ax3d, rect.x, rect.y, z_offset, rect.width, rect.height, carton_height, color)
    ...
    if cartons_per_layer > 0:
        ...
        volume_utilization = (carton_area * carton_height * max_layers / stack_volume) * 100
    else:
        area_utilization = 0
    ...
    with col2:
        ...
        st.write(f"**Area Utilization:** {area_utilization:.1f}%")
