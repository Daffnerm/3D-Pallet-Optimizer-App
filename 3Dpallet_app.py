...
def draw_box(ax, x, y, z, dx, dy, dz, color):
    ...
    # Define the faces using vertices indices
    faces = [
    ...
    poly = Poly3DCollection(faces, alpha=1.0)
...
if carton_height > max_height:
    ...
    with vis_col2:
        # 3D Visualization (FIXED PALLET POSITIONING)
        if cartons_per_layer > 0:
            # Create 3D plot
            ...
            # Set axis limits FIRST for proper scaling
            ax3d.set_xlim(0, PALLET_LENGTH)
            ax3d.set_ylim(0, PALLET_WIDTH)
            ax3d.set_zlim(0, total_height)
            
            # Draw the pallet base at the bottom (z=0)
            draw_box(ax3d, 0, 0, 0, PALLET_LENGTH, PALLET_WIDTH, PALLET_HEIGHT, '#C0A080')
            
            # Draw cartons ABOVE the pallet
            for layer in range(max_layers):
                z_pos = PALLET_HEIGHT + (layer * carton_height)  # Start at top of pallet
                
                for pos in positions:
                    # Get dimensions based on carton orientation
                    if (carton_length == 17 and carton_width == 13) or (carton_length == 13 and carton_width == 17):
                        dx = pos["w"]
                        dy = pos["h"]
                    else:
                    ...
                    # Draw carton above the pallet
                    draw_box(ax3d, pos["x"], pos["y"], z_pos, dx, dy, carton_height, color)
    ...
    total_pallet_volume = PALLET_LENGTH * PALLET_WIDTH * total_height
    if total_pallet_volume > 0:
        volume_utilization = (carton_volume / total_pallet_volume) * 100
    else:
    ...
    if st.button("Save Configuration to Excel"):
        df = pd.DataFrame([{
            ...
            "Layers": max_layers,
            "Total": total_cartons,
            "Area Utilization (%)": round(utilization_percentage, 1),
