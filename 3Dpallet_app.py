...
with col1:
    carton_length = st.number_input("Carton Length (inches)", min_value=1, step=1, format="%d")
    carton_width = st.number_input("Carton Width (inches)", min_value=1, step=1, format="%d")
with col2:
    carton_height = st.number_input("Carton Height (inches)", min_value=1, step=1, format="%d")
...
if carton_height > max_height:
        ...
        if cartons_per_layer > 0:
            if (carton_length == 17 and carton_width == 13) or (carton_length == 13 and carton_width == 17):
                ...
                for i in range(cols):
                    for j in range(rows):
                        positions.append({
                            "x": i * used_l, "y": j * used_w, "w": used_l, "h": used_w
                        })
                        rect = plt.Rectangle(
                        ...
                        ax.add_patch(rect)
                        ax.text(
                            i * used_l + used_l/2, j * used_w + used_w/2, str(i * rows + j + 1),
                            ha='center', va='center', color='white', fontweight='bold'
                        )
        else:
    ...
    with vis_col2:
        ...
        if cartons_per_layer > 0:
            ...
            # Draw the pallet base (wood color)
            draw_box(ax3d, 0, 0, 0, PALLET_LENGTH, PALLET_WIDTH, 4, '#C0A080')
            # For each layer
            for layer in range(max_layers):
                z_offset = 4 + layer * carton_height  # Start above the pallet base
                for pos in positions:
                    ...
                    draw_box(ax3d, pos["x"], pos["y"], z_offset, dx, dy, dz, color)
            ax3d.set_xlim(0, PALLET_LENGTH)
            ax3d.set_ylim(0, PALLET_WIDTH)
            ax3d.set_zlim(0, 4 + max_layers * carton_height)
