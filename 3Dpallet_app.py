...
PALLET_LENGTH = 48
PALLET_WIDTH = 40
PALLET_HEIGHT = 4  # Height of the pallet base (used for utilization, not for 3D viz)
...
if carton_height > max_height:
        ...
        if cartons_per_layer > 0:
            if (carton_length == 17 and carton_width == 13) or (carton_length == 13 and carton_width == 17):
                for i, pos in enumerate(positions):
                    ...
                    ax.add_patch(rect)
                    ax.text(
                        pos["x"] + pos["w"]/2, pos["y"] + pos["h"]/2, str(i+1),
            ...
            ax.text(PALLET_LENGTH/2, PALLET_WIDTH/2, "Carton too large!", ha='center', va='center', color='red', fontsize=14)
            ...
            for layer in range(max_layers):
                ...
                for pos in positions:
                    plot_cube(
                        ...
                        dx=pos["w"], dy=pos["h"], dz=carton_height,
                        color=color
                    )
    ...
    if cartons_per_layer > 0:
        ...
        volume_utilization = (carton_volume / stack_volume) * 100 if stack_volume > 0 else 0
    else:
        area_utilization = 0
