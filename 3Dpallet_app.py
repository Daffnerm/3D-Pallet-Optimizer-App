...
# COMPLETELY NEW 3D box drawing function
def draw_box(ax, x, y, z, dx, dy, dz, color):
    ...
    # Create the cube
    poly3d = Poly3DCollection(faces, alpha=1.0)
...
if carton_height > max_height:
    ...
    # Generate carton positions for both 2D and 3D visualizations
    positions = []
    if cartons_per_layer > 0:
        # Special case for 17×13 cartons
        if (carton_length == 17 and carton_width == 13) or (carton_length == 13 and carton_width == 17):
            ...
            # Standard positioning for other carton sizes
    ...
    # 3D VISUALIZATION - COMPLETELY REWRITTEN
    with vis_col2:
        if cartons_per_layer > 0:
            # Create figure
            ...
            # Calculate total stack height including pallet
            ...
            # Draw the wooden pallet base FIRST
            draw_box(ax3d, 0, 0, 0, PALLET_LENGTH, PALLET_WIDTH, PALLET_HEIGHT, '#C0A080')
            
            # Draw cartons layer by layer
            for layer in range(max_layers):
                # Starting z-coordinate for this layer = pallet height + (layer number * carton height)
                z_start = PALLET_HEIGHT + (layer * carton_height)
                
                # Alternate colors by layer for better visibility
                ...
                for pos in positions:
                    draw_box(
                        ...
                        pos["x"],                # x-coordinate
                        pos["y"],                # y-coordinate
                        z_start,                 # z-coordinate (starts at top of pallet)
                        pos["w"],                # width of carton
                        pos["h"],                # height of carton (in x-y plane)
                        carton_height,           # vertical height of carton
                        layer_color              # color based on layer number
                    )
    ...
    if cartons_per_layer > 0:
        ...
        # Calculate volume utilization (excluding pallet base volume)
        if stack_volume > 0:
