...
PALLET_LENGTH = 48
PALLET_WIDTH = 40
PALLET_HEIGHT = 4  # Height of the pallet base (inches)
...
if carton_height > max_height:
    ...
    with vis_col2:
        # 3D Visualization (fixed pallet position)
        if cartons_per_layer > 0:
            fig_3d = plt.figure(figsize=(6, 6))
            ax3d = fig_3d.add_subplot(111, projection='3d')
            
            # Draw pallet base at bottom (z=0 to z=4)
            draw_box(ax3d, 0, 0, 0, PALLET_LENGTH, PALLET_WIDTH, PALLET_HEIGHT, '#C0A080')
            
            # Draw carton layers
            for layer in range(max_layers):
                z_offset = PALLET_HEIGHT + (layer * carton_height)  # Start above pallet
    ...
    # Calculate pallet area utilization
